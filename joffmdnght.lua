--[[
  JOSEPEDOV V55 — MIDNIGHT CHASERS
  Highway AutoRace exploit | Fluent UI | Ultimate Edition

  ══════════════════════════════════════════════════════════════
  V55 FIX — UNCOMPRESSED & MOBILE MOTO BOOST
  ══════════════════════════════════════════════════════════════
  - FULLY UNCOMPRESSED: Restored the original 1400+ line structure 
    to guarantee flawless execution. Removed all semicolon compression.
  - Removed Gemini AI & HTTP Services completely.
  - Fixed "Infinite Cruising": Added a dedicated on-screen Mobile 
    Moto Button. The bike ONLY boosts when this is physically held.
  - Added a x2.5 torque multiplier so Moto Boost feels responsive.
  ══════════════════════════════════════════════════════════════
]]
...
local guiTarget = (type(gethui) == "function" and gethui()) or (pcall(function() return game:GetService("CoreGui") end) and CoreGui) or player:WaitForChild("PlayerGui")
...
subLbl.Text = "JOSEPEDOV V55  ·  UNCOMPRESSED EDITION"
...
local function SetProg(pct, msg, activeDot)
    TweenService:Create(barFill, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(pct / 100, 0, 1, 0)}):Play()
    barTxt.Text = string.format("  %d%%  —  %s", math.floor(pct), msg)
    
    local ci = math.max(1, math.min(#CAM_ROUTE, math.round(pct / 100 * #CAM_ROUTE + 0.5)))
    TweenService:Create(cam, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {CFrame = CAM_ROUTE[ci][1]}):Play()
...
-- ─────────────────────────────────────────────────────────────
--  GAME CONTENT PRELOADER (V51)
-- ─────────────────────────────────────────────────────────────
SetProg(5, "Preloading Game Assets...", 1)
task.wait(0.1)

local GAME_ASSETS = {
    "rbxassetid://14596824377", "rbxassetid://15694239990", "rbxassetid://14916031176", "rbxassetid://14916031762",
    "rbxassetid://14573638321", "rbxassetid://3557627978", "rbxassetid://15179434724", "rbxassetid://17363357783",
    "rbxassetid://15612489676", "rbxassetid://15618247443", "rbxassetid://14321936910", "rbxassetid://17532595840",
    "rbxassetid://9032941519", "rbxassetid://14925232957", "rbxassetid://15617404429", "rbxassetid://16312766199",
    "rbxassetid://9476531852", "rbxassetid://15388429560", "rbxassetid://15449028050", "rbxassetid://8396779051",
    "rbxassetid://11985298197", "rbxassetid://14084476116", "rbxassetid://8709286869", "rbxassetid://10491935094",
    "rbxassetid://17043030320", "rbxassetid://1847258023", "rbxassetid://15082168287", "rbxassetid://16919833005",
    "rbxassetid://15694240604", "rbxassetid://15694240576", "rbxassetid://15694240587", "rbxassetid://15694240578"
}
...
pcall(function()
    ContentProvider:PreloadAsync(GAME_ASSETS, function(assetId, status)
        loaded = loaded + 1
        local pct = 5 + (loaded / total) * 35
        SetProg(pct, string.format("Preloading... %d/%d", loaded, total), 1)
    end)
end)

SetProg(40, "Streaming Map Areas...", 2)

local STREAM_POSITIONS = {
    Vector3.new(2464, 5, 583),
    Vector3.new(2663, 5, 389),
    Vector3.new(2900, 5, 700),
    Vector3.new(3087, 5, 636),
    Vector3.new(3260, 12, 1016),
    Vector3.new(3485, 5, 622)
}

for si, spos in ipairs(STREAM_POSITIONS) do
    local spct = 40 + (si / #STREAM_POSITIONS) * 15
    SetProg(spct, string.format("Streaming Area %d/%d", si, #STREAM_POSITIONS), 2)
    pcall(function() 
        player:RequestStreamAroundAsync(spos, 5) 
    end)
end
...
--  CONFIG & STATE
-- ─────────────────────────────────────────────────────────────
SetProg(60, "Loading Configurations...", 3)
task.wait(0.2)
...
-- ─────────────────────────────────────────────────────────────
SetProg(65, "Building Infinite Sky Road...", 3)
task.wait(0.3)
...
--  WORLD MODIFIERS
-- ─────────────────────────────────────────────────────────────
SetProg(70, "Restoring World Modifiers...", 4)
task.wait(0.2)
    ...
    if Config.TrafficBlocked then
        pcall(function()
            if ev and getconnections then 
                for _, c in pairs(getconnections(ev.OnClientEvent)) do 
                    c:Disable() 
                end 
            end
        end)
...
local function ManagePhysicsMods(car)
    ...
    if Config.TireGrip then
        ...
        vf.Force = Vector3.new(0, -mass * Workspace.Gravity * 2.0, 0)
    else
        if vf then 
            vf:Destroy() 
        end
...
local function DoRaceLoop(uuidFolder)
    ...
    while Config.AutoRace and AR_STATE == "RACING" do
        ...
        while tick() < flyLimit do
            ...
            local desiredVelY = yErr * 4  

            local ceilRay = Workspace:Raycast(myPos, Vector3.new(0, 25, 0), rcParams)
            ...
            if ceilRay then
                ...
                if myPos.Y >= ceilSafeY then
                    ...
                    desiredVelY = math.min(desiredVelY, (ceilSafeY - myPos.Y) * 4)
                end
        ...
        if cpConn then 
            pcall(function() cpConn:Disconnect() end) 
        end
...
-- ── MOBILE MOTO BOOST BUTTON ──
local MobileMotoBtn = Instance.new("TextButton", ScreenGui)
MobileMotoBtn.Size = UDim2.new(0, 80, 0, 80)
MobileMotoBtn.Position = UDim2.new(1, -100, 1, -120)
MobileMotoBtn.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
MobileMotoBtn.Text = "🏍️\nMOTO"
MobileMotoBtn.Font = Enum.Font.GothamBold
MobileMotoBtn.TextSize = 14
MobileMotoBtn.TextColor3 = Theme.Accent
MobileMotoBtn.Visible = false

local MobileMotoBtnCorner = Instance.new("UICorner", MobileMotoBtn)
MobileMotoBtnCorner.CornerRadius = UDim.new(1, 0)

local MobileMotoBtnStroke = Instance.new("UIStroke", MobileMotoBtn)
MobileMotoBtnStroke.Color = Theme.Accent
MobileMotoBtnStroke.Thickness = 2

local isMobileMotoHeld = false

MobileMotoBtn.InputBegan:Connect(function(input) 
    if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then 
        isMobileMotoHeld = true 
        MobileMotoBtn.BackgroundColor3 = Theme.Button
    end 
end)

MobileMotoBtn.InputEnded:Connect(function(input) 
    if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then 
        isMobileMotoHeld = false 
        MobileMotoBtn.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
    end 
end)
...
TitleLbl.Text = "🏁  MIDNIGHT CHASERS  V55"
...
arSub.Text = "City Highway Race  ·  V55"
...
Section(TabCar, "  CAR ENGINE MODS")
FluentToggle(TabCar, "⚡ Car Speed Hack", "Overrides car's normal engine max speed", function(v) 
    Config.SpeedHack = v
    return v 
end)
...
Section(TabCar, "  CAR HACK TUNING")
...
-- ── MOTORCYCLE SECTION (V55 FIX) ──────────────────────────────
Section(TabCar, "  MOTORCYCLE MODS")

FluentToggle(TabCar, "🏍️ Moto Speed Boost", "Hold W (PC) or the UI Moto Button (Mobile) to boost bikes.", function(v)
    Config.MotoSpeedHack = v
    if UserInputService.TouchEnabled then
        MobileMotoBtn.Visible = v
    end
    return v
end)

FluentToggle(TabCar, "🛡️ No Crash Death", "Health stays full — no death or ragdoll on collision", function(v)
...
Section(TabCar, "  MOTO HACK TUNING")
...
InfoRow(TabMisc, "🏁  Midnight Chasers AutoRace  V55")
InfoRow(TabMisc, "⚡  Pure Uncompressed Edition")
InfoRow(TabMisc, "💡  Fluent UI  ·  josepedov")
InfoRow(TabMisc, "📋  Changelog: Flawless Mobile Moto Controls.")
...
RunService.Heartbeat:Connect(function()
    ...
    local gasVal = currentSeat.ThrottleFloat or 0
    local brakeVal = 0
    local gearVal = 1
    ...
    if iface and iface:FindFirstChild("Values") then
        ...
        if v:FindFirstChild("Throttle") then 
            gasVal = v.Throttle.Value 
        end
        if v:FindFirstChild("Brake") then 
            brakeVal = v.Brake.Value 
        end
        if v:FindFirstChild("Gear") then 
            gearVal = v.Gear.Value 
        end
    end
    ...
    -- ── CAR SPEED HACK OVERRIDE ───────────────────────────────────
    if Config.SpeedHack and not IsMoto(currentCar) then
        if root and root:IsA("BasePart") then
    ...
    -- ── MOTO SPEED HACK (V55 MOBILE BUTTON FIX) ────────────────────
    elseif Config.MotoSpeedHack and IsMoto(currentCar) then
        if root and root:IsA("BasePart") then
            ...
            -- HARDWARE POLLING: PC W/UP keys
            local isThrottling = false
            pcall(function()
                isThrottling = UserInputService:IsKeyDown(Enum.KeyCode.W) or UserInputService:IsKeyDown(Enum.KeyCode.Up)
            end)
            
            -- MOBILE OVERRIDE: Button Held
            if isMobileMotoHeld then 
                isThrottling = true 
            end

            if isThrottling and not isRev then
                if xzSpeed < Config.MotoMaxSpeed then
                    -- 2.5x Multiplier to counteract heavy bike drag
                    root.AssemblyLinearVelocity = vel + flat * (Config.MotoAccel * 2.5)
                    SetStatus(string.format("🏍️ Moto Boost: %.0f / %d st/s", xzSpeed, Config.MotoMaxSpeed), 0, 215, 80)
                else
                    SetStatus(string.format("🏍️ Moto Boost: MAX  %.0f st/s", xzSpeed), 255, 200, 0)
                end
            else
...
print("\n═════════════════════════════════════════════════")
print("[J55] Midnight Chasers — V55 Uncompressed Ready")
print("[J55] Developed by josepedov")
print("[J55] Full robust execution completed.")
print("═════════════════════════════════════════════════\n")
