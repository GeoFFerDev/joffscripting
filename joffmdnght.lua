--[[
  ...
  Root cause: floor raycast occasionally returned nil → ceiling guard pushed car underground.
  V23c fixes:
  • Floor raycast -250 studs + fallback to lastSafeY
  • Gentle ceiling push (*1.2) + extra buffer
...
vig.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(0,0,0)), ColorSequenceKeypoint.new(0.5, Color3.fromRGB(6,8,14)), ColorSequenceKeypoint.new(1, Color3.fromRGB(0,0,0))}
vig.Rotation = 45
vig.Transparency = NumberSequence.new{NumberSequenceKeypoint.new(0, 0.6), NumberSequenceKeypoint.new(0.5, 0), NumberSequenceKeypoint.new(1, 0.6)}
...
local Config = {
    SpeedHack = false, AutoRace = false, InfNitro = false,
    TrafficBlocked = false, FPS_Boosted = false, FullBright = false,
    Acceleration = 3.0, MaxSpeed = 320, AutoRaceSpeed = 350, Deadzone = 0.1
}
...
local function FindNextCP(raceFolder, clearedSet, skipIdx)
    ...
    for _,child in ipairs(cpVal:GetChildren()) do
        if child:IsA("BasePart") then
            ...
            if idx and idx < bestIdx and idx \~= skipIdx and not (clearedSet and clearedSet[idx]) then
...
-- ─────────────────────────────────────────────────────────────
--  STABLE FLIGHT ENGINE — V23c ULTRA STABLE
-- ─────────────────────────────────────────────────────────────
local ROAD_HOVER       = 5.5
local GATE_INSIDE      = 0.30
local PUNCH_DIST       = 14
local CEIL_GAP         = 7
local FLOOR_RAY_DIST   = 250
    ...
    while Config.AutoRace and AR_STATE == "RACING" do
        ...
        while tick() < flyLimit do
            ...
            if distXZ <= PUNCH_DIST then
                ...
                local punchCF = CFrame.new(gatePart.Position.X, gateTargetY, gatePart.Position.Z) * CFrame.Angles(0, ry, 0)
                pcall(function() car:PivotTo(punchCF) end)
            ...
            -- Y COMPUTATION — V23c ULTRA STABLE
            ...
            if floorRay then
                ...
                roadY = lastSafeY - ROAD_HOVER
            end

            targetY = math.max(targetY, roadY + ROAD_HOVER)
            if myPos.Y < roadY + 2 then targetY = roadY + ROAD_HOVER + 8 end
            ...
            if ceilHit then
                ...
                if gap < CEIL_GAP then
                    local headroom = roadY and (ceilY - roadY) or 30
                    if headroom > MIN_HEADROOM then
                        local pushDown = myPos.Y - (CEIL_GAP - gap) * 1.2
                        pushDown = math.max(pushDown, roadY + ROAD_HOVER + 2)
                        targetY = math.min(targetY, pushDown)
                    else
                        local topRay = Workspace:Raycast(Vector3.new(myPos.X, ceilY + 1, myPos.Z), Vector3.new(0, 100, 0), rcParams)
                        if topRay then
            ...
            local Kp, Kd, maxVY
            if yErr < -8 then Kp=10; Kd=2; maxVY=120
            elseif yErr < -3 then Kp=7; Kd=3; maxVY=60
            elseif yErr > 10 then Kp=8; Kd=2; maxVY=90
            elseif yErr > 3 then Kp=5; Kd=3; maxVY=45
            else Kp=3; Kd=5; maxVY=15 end
            ...
            SetStatus(string.format("→ CP #%d  %.0f studs  Y%.1f▶%.1f", cpIdx, distXZ, myPos.Y, gateTargetY), 0, 190, 255)
            task.wait()
        end
...
local function ToggleTraffic()
    ...
    if Config.TrafficBlocked then
        ...
        for _,n in ipairs({"NPCVehicles","Traffic","Vehicles"}) do local f=Workspace:FindFirstChild(n); if f then f:ClearAllChildren() end end
    else
    ...
    pcall(function()
        if Config.FPS_Boosted then
            ...
            for _,v in ipairs(workspace:GetDescendants()) do pcall(function()
                if v:IsA("BasePart") then v.CastShadow=false elseif v:IsA("ParticleEmitter") or v:IsA("Trail") then v.Enabled=false end
            end) end
        else
            Lighting.GlobalShadows = true
            if sethiddenproperty then sethiddenproperty(Lighting,"Technology",OriginalTech) end
            for _,v in ipairs(workspace:GetDescendants()) do pcall(function()
                if v:IsA("BasePart") then v.CastShadow=true elseif v:IsA("Trail") or v:IsA("ParticleEmitter") then v.Enabled=true end
            end) end
        end
    end)
...
-- FLUENT UI (full unchanged block)
local Theme = {
...
local function FluentSlider(parent, label, minV, maxV, defaultV, sweetspot, getV, setV)
    ...
    local function updateFromPct(pct)
        ...
        local col = val >= maxV and Theme.Red or (sweetspot and val > sweetspot and Theme.Orange or Theme.Accent)
...
FluentToggle(TabCar, "⚡ Speed Hack", "Override car top speed", function(v) Config.SpeedHack=v; return v end)

Section(TabCar, "  NITRO")
FluentToggle(TabCar, "🔥 Infinite Nitro", "Keeps CurrentBoost at MaxBoost", function(v) Config.InfNitro=v; return v end)

Section(TabCar, "  TUNING")
FluentStepper(TabCar, "Top Speed (SpeedHack)", "%d st/s", function() return Config.MaxSpeed end,
    function() Config.MaxSpeed=math.max(50,Config.MaxSpeed-50) end,
    function() Config.MaxSpeed=Config.MaxSpeed+50 end)
FluentStepper(TabCar, "Acceleration", "%.1f", function() return Config.Acceleration end,
...
InfoRow(TabMisc, "🔧  Gate-inside Y · Punch-through · 250-stud floor")
InfoRow(TabMisc, "🎚️  Gentle ceiling + absolute safety net")
InfoRow(TabMisc, "💡  Fluent UI  ·  josepedov")
InfoRow(TabMisc, "📋  V23c — car can no longer go underground")
...
RunService.Heartbeat:Connect(function()
    ...
    if Config.InfNitro then
        ...
        if valObj then
            ...
            if maxB and maxB > 0 then valObj:SetAttribute("CurrentBoost", maxB) end
        end
    end
...
print("[J23c] Midnight Chasers — V23c ULTRA STABLE ready")
print("[J23c] Car can no longer go under the road — guaranteed checkpoint hits")
